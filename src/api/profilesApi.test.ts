...
describe('updateProfile', () => {
  it('sends PUT with bearer token and returns updated profile', async () => {
    mockFetch.mockResolvedValueOnce({ ok: true, json: async () => sampleProfile })
    const data = { displayName: 'New Name', languages: [], platforms: ['PC'], genres: [], externalLinks: {} }
  ...
  it('throws ApiError on 400', async () => {
    ...
    await expect(updateProfile('tok', { displayName: '', languages: [], platforms: [], genres: [], externalLinks: {} })).rejects.toBeInstanceOf(ApiError)
  })
})
